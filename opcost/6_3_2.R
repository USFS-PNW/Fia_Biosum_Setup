...
yarderEst<-function(m,p){
  (yarderTime(m)*p["yarder",])+(sawTime(m)*p["chainsaw",])+(slideTime(m)*p["slideboom",])+((chipTime(m)*.25)*p["chipper",])
}
...
sawSkidEst<-function(m,p){
  (skidderTime(m)*p["grappleSkidderLarge",]+(sawTime(m)*p["chainsaw",])+(slideTime(m)*p["slideboom",]))
}

sawSkidLogEst<-function(m,p){
  ((skidderTime(m)*p["grappleSkidderLarge",]*2.5)+(sawTime(m)*p["chainsaw",]*2.5)+(slideTime(m)*p["slideboom",]))
}

harForEst<-function(m,p){
  (((forTime(m)*.75)+2)*p["forwarder",])+((harTime(m)*p["harvester",])*5)+(slideTime(m)*p["slideboom",])+((chipTime(m)*.25)*p["chipper",])
}
...
yarderHarEst<-function(m,p){
  (yarderTime(m)*p["yarder",])+(harTime(m)*p["harvester",])
}

fbSkidEst<-function(m,p){
  (fbTime(m)*p["fellerBuncher",])+((skidderTime(m)*1.5*p["grappleSkidderLarge",])*1.3)+(slideTime(m)*p["slideboom",])
}
...
m5<-prescribe(m,m3)+mim(m,m3)

m5.1<-round(m5, 2)

m6<-data.frame(m$"Stand", m$"YearCostCalc", m5, labelEq1(m), m$"RxPackage_Rx_RxCycle")
