...
chipTrees<-function(m){
  ifelse(m$"Harvesting.System"=="Ground-Based Mech WT"|m$"Harvesting.System"=="Ground-Based Man WT",
...
labelEq1<-function(m){
  ifelse((m$"Harvesting.System")=="Cable Manual WT", "Cable Manual WT", 
         ifelse((m$"Harvesting.System")=="Ground-Based Man WT", "Ground-Based Man WT",
                                            ...
                                            ifelse((m$"Harvesting.System")=="Cable Manual Log", "Cable Manual Log",
                                                   ifelse((m$"Harvesting.System")=="Ground-Based Manual Log", "Ground-Based Manual Log", 
                                                          ifelse((m$"Harvesting.System")=="Helicopter Manual WT", "Helicopter Manual WT", 
                                                                 ifelse((m$"Harvesting.System")=="Cable CTL", "Cable CTL", 1))))))))))
}
...
miclb<-function(m){ifelse(m$Harvesting.System=="Cable Manual WT"|
                            ...
                            m$Harvesting.System=="Cable Manual Log"|
                            m$Harvesting.System=="Ground-Based Manual Log"|
                            m$Harvesting.System=="Helicopter Manual WT", (((m$Move_In_Hours*100)*2)/m$Harvest_Area_Assumed_Acres),
...
m5<-prescribe(m,m3)+mim(m,m3)+miclb(m)

m5.1<-round(m5, 2)

m6<-data.frame(m$"Stand", m$"YearCostCalc", m5, labelEq1(m), m$"RxPackage_Rx_RxCycle", m$"biosum_cond_id", m$"RxPackage", m$"Rx", m$"RxCycle")

m7<-m6

colnames(m7)<-c("stand", "rx_year", "harvest_cpa", "harvest_system", "RxPackage_Rx_RxCycle", "biosum_cond_id","RxPackage","Rx","RxCycle")

exp2<-subset(m7, harvest_cpa<1 | harvest_cpa=="NA" | harvest_cpa=="Inf")

m16<-data.frame(m$"Stand", m$"YearCostCalc", labelEq3(m,m3), labelEq2(m,m3), ifelse(labelEq3(m,m3)==labelEq1(m),"TRUE","FALSE"),m$"RxPackage_Rx_RxCycle",
...
colnames(m16)<-c("stand", "rx_year", "harvest_system", "Harvest_CPA", "Matches Original System?","RxPackage_Rx_RxCycle",
...
m22<-data.frame(m$"Stand",m$"YearCostCalc",labelEq1(m),chippingCost2(m,m3),miclb(m),m$"RxPackage_Rx_RxCycle", m$"biosum_cond_id",
                m$"RxPackage", m$"Rx", m$"RxCycle")

print("m22:OK")

colnames(m22)<-c("stand","rx_year","harvest_system","chip_cost","Assumed Move In Costs","RxPackage_Rx_RxCycle",
                 "biosum_cond_id", "RxPackage", "Rx", "RxCycle")

m222<-subset(m22, chip_cost=="NA" | chip_cost=="Inf")

m22<-m22[! rownames(m22) %in% rownames(m222),]

row.names(m22)<-1:nrow(m22)
...
colnames(exp22)<-c("stand", "rx_year", "harvest_cpa", "harvest_system","RxPackage_Rx_RxCycle",
