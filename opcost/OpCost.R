...
con<-odbcConnectAccess2007(package.file)
##con2<-odbcConnectAccess2007("C:/fia_biosum/Output_Testing8cycle/db/master.mdb")
...
m3<-data.frame(row.names=c("fellerBuncher","forwarder","harvester","grappleSkidderLarge",
                           "cableSkidderLarge","wheelFellerBuncher","crawlerFellerBuncher","yarder","slideboom","chainsaw","chipper"),values=c(210.10,170.00,180.00,110.00,149.80,205.10,185.60,299.00,200.00,90.00,100.00))


#####Run Data

prescribe<-function(m,m3){ifelse((m$"Harvesting.System")=="Cable Manual WT", yarderEst(m,m3),
                                 ifelse((m$"Harvesting.System")=="Ground-Based Man WT", harSkidEst(m,m3),
                                        ifelse((m$"Harvesting.System")=="Ground-Based Mech WT", fbSkidEst(m,m3),
                                               ifelse((m$"Harvesting.System")=="30500", harForEst(m,m3), 1))))
}
...
twitchVolM<-function(m){
  ((m$"Large.log.trees.per.acre"*m$"Large.log.trees.average.vol.ft3.")+(m$"Small.log.trees.per.acre"*m$"Small.log.trees.average.volume.ft3.")+(m$"Chip.tree.per.acre"*m$"Chip.trees.average.volume.ft3"))/
    (m$"Large.log.trees.per.acre"+m$"Small.log.trees.per.acre"+m$"Chip.tree.per.acre")*0.0283168
}

twitchWeight<-function(m){
  twitchDF<-data.frame(m[,"Large.log.trees.average.density.lbs.ft3."], m[,"Small.log.trees.average.density.lbs.ft3."])
...
boldingHar=function(m){
  .1765521+(0.003786057*dbh(m))+(4.936639*m$"Percent.Slope"*(1/sqrt(treesRemoved(m))))
} ##****
...
karhaHar<-function(m){
  0.288+(0.1004*(twitchVolM(m)/0.001))+(-0.00008*(twitchVolM(m)/0.001)^2)
}  ##m^3/PMH

karhaHar2<-function(m){
  0.181+(0.1315*(twitchVolM(m)*1000))
}
...
boldingFor<-function(m){
  .1761+(0.00357*dbh(m))+(4.93*m$"Percent.Slope"*(1/sqrt(treesRemoved(m))))
}
...
jirousekFor3<-function(m){
  17.0068*(m$"One.way.Yarding.Distance"^(13.2533/m$"One.way.Yarding.Distance"))
}
...
flattenHeli<-function(m){
  40.737274+(0.0168951*m$"One.way.Yarding.Distance")+(2.052894*twitchWeight(m))
}

dystraHeli<-function(m){
  1.3721152+0.0126924*m$"Percent.Slope"+0.000246741*(m$"One.way.Yarding.Distance")+0.031200*treesRemoved(m)
}

cutisHeli<- function(m){ 
...
ghafSkidTime<-function(m){
  ifelse(m$"Percent.Slope"<26, totalVolM(m)/(twitchVolM(m)*30)*ghafSkid(m)/60, NA)
}
...
hansillChip2Time<-function(m){
  ifelse(dbh.cm(m)<76, ((chipTrees(m)*hansillChip2(m))/60/60), NA)
} ##Hours/Acre

curtisYardingTime<-function(m){
  treesRemoved(m)/curtisYarding(m)
} ## Hours/Acre

manBC2<-function(m){
  ifelse(is.na(m$BrushCutTpa), 0, ifelse(m$BrushCutTpa>0 ,ifelse(m$BrushCutAvgVol<4, m$BrushCutTpa/(60), m$BrushCutTpa/(2*60))))
}

curtisYarding2Time<-function(m){
  treesRemoved(m)/curtisYarding2(m)
} ## Hours/Acre

fisherYarderTime<-function(m){
  ((((m$"Small.log.trees.per.acre"+m$"Large.log.trees.per.acre")/1.35)*fisherYarder(m))/60)
} ##  Hours/Acre

kluenderSkidTime<-function(m){
  ifelse(m$"Percent.Slope"<30, kluenderSkid(m), NA)
}
...
dystraHeliTime<-function(m){
  ifelse((twitchWeight(m)<2900), totalVolM(m)/(dystraHeli(m)*2), NA)
} ##Hours/Acre

flattenHeliTime<-function(m){
  ifelse((twitchWeight(m)<2900), flattenHeli(m)*(treesRemoved(m)*.75)/(60^3), NA)
} ## Hours/Acre

ghafSawTime<-function(m){
  ifelse(dbh.cm(m)>25, ((ghafSaw(m)*treesRemoved(m))/60), NA)
}
...
jirousekFor3Time<-function(m){
  ifelse(m$"Percent.Slope"<40, totalVolM(m)/jirousekFor3(m), NA)
}

boldingChipTime<-function(m){
  ifelse(dbh.cm(m)<50, ((boldingChip(m))/60), NA)
}

akaySkidTime<-function(m){
  ifelse(m$"One.way.Yarding.Distance"<20, akaySkid(m)*.4, NA)
}

jirousekFor2Time<-function(m){
  ifelse(m$"Percent.Slope"<40, totalVolM(m)/jirousekFor2(m), NA)
}
...
cutisHeliTime<-function(m){
  ifelse(dbh.cm(m)>20, cutisHeli(m)/60, NA)
}
...
boldingForTime<-function(m){
  ifelse(m$"Percent.Slope"<40, totalVolM(m)/boldingFor(m), NA)
}
...
klepacSkidTime<-function(m){
  ifelse(m$"Percent.Slope"<10, treesRemoved(m)/(klepacSkid(m)*15), NA)
}
...
stokesFBTime<-function(m){
  (ifelse(m$"Percent.Slope"<30, stokesFB(m)/60, NA))
} ##Hours/Acre
...
karhaHarTime<-function(m){
  ifelse(twitchVolM(m)<.3, (totalVol(m)*0.0283168)/karhaHar(m), NA)
} ## Hours/Acre
...
yarderEst<-function(m,p){
  (yarderTime(m)*p["yarder",])+(sawTime(m)*p["chainsaw",])+(slideTime(m)*p["slideboom",])
}

harForEst<-function(m,p){
  (forTime(m)*p["forwarder",])+(harTime(m)*p["harvester",])+(slideTime(m)*p["slideboom",])
}

harSkidEst<-function(m,p){
  (harTime(m)*p["harvester",])+(skidderTime(m)*p["grappleSkidderLarge",])+(slideTime(m)*p["slideboom",])
}

fbSkidEst<-function(m,p){
  (fbTime(m)*p["fellerBuncher",])+(skidderTime(m)*p["grappleSkidderLarge",])+(slideTime(m)*p["slideboom",])
}
...
labelEq1<-function(m){
  ifelse((m$"Harvesting.System")=="Cable Manual WT", "Yarder System", 
         ifelse((m$"Harvesting.System")=="Ground-Based Man WT", "Harvester Skidder WT",
                ifelse((m$"Harvesting.System")=="Ground-Based Mech WT", "Fellerbuncher Skidder WT",
                       ifelse((m$"Harvesting.System")==30500, "Harvester Forwarder CTL", 1))))
}
...
labelEq2<-function(m,p){
  ifelse(m$"Percent.Slope">45.001, "Yarder System", ifelse(
    harForEst(m,p) & harSkidEst(m,p)>fbSkidEst(m,p), "Fellerbuncher Skidder WT", ifelse(
      fbSkidEst(m,p) & harForEst(m,p)>harSkidEst(m,p), "Harvester Skidder WT", ifelse(
        harSkidEst(m,p) & fbSkidEst(m,p)>harForEst(m,p), "Harvester Forwarder CTL", NULL))))
}
...
m6<-data.frame(m$"Stand", m$"Year", m5, labelEq1(m), m$"RxPackage_Rx_RxCycle")

colnames(m6)<-c("Stand ID", "Year", "Treatment Cost (Per Acre)", "Treatment Selection", "RxPackage_Rx_RxCycle")

m7<-m6

colnames(m7)<-c("stand", "rx_year", "harvest_cpa", "harvest_system", "RxPackage_Rx_RxCycle")
                     ...
                     ifelse(exp21$exp2.harvest_cpa=="NaN" & exp21$Small.log.trees.average.volume.ft3.==0 & exp21$Large.log.trees.average.vol.ft3.==0 & exp21$Harvesting.System=="Ground-Based Mech WT", "Completely Zeroed Logs:GB",
                            ifelse(exp21$exp2.harvest_cpa=="NaN" & exp21$Small.log.trees.average.volume.ft3.==0 & exp21$Large.log.trees.average.vol.ft3.==0 & exp21$Harvesting.System=="Cable Manual WT", "Completely Zeroed Logs:CB",
                                   ifelse(exp21$exp2.harvest_cpa<0 & exp21$Harvesting.System=="Ground-Based Mech WT", "Check Skidding Times",
                                          ifelse(exp21$exp2.harvest_cpa<0 & exp21$Harvesting.System=="Cable Manual WT", "Check Short Yarding Distances", "Oh Yay, a new one!"))))))
...
sqlSave(con, exp22, tablename="OpCost_err", append=TRUE)

sqlSave(con, m10, tablename="OpCost_ouput", append=TRUE)
