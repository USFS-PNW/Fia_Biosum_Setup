@ECHO OFF
TITLE = FIA_Tree Volume and Biomass Compiler

SET SHOWMSG=%1

IF EXIST "%APPDATA%"\FIABiosum\tvbc_error_msg.txt DEL "%APPDATA%"\FIABiosum\tvbc_error_msg.txt

IF NOT EXIST "%JAVA_HOME%"\bin\JAVA.EXE GOTO tryagain

REM Update the tryagain section if the arguments change
CALL "%JAVA_HOME%"\bin\JAVA.EXE -jar "%APPDATA%"/FIABiosum/FIA_TreeVBC.jar -i "%APPDATA%"/FIABiosum/tvbc_tree_data.db -it tvbc_tree_data ^
-o "%APPDATA%"/FIABiosum/tvbc_tree_data.db -ot tvbc_tree_data_calc
...
:tryagain
CALL JAVA.EXE -jar  "%APPDATA%"/FIABiosum/FIA_TreeVBC.jar -i "%APPDATA%"/FIABiosum/tvbc_tree_data.db -it tvbc_tree_data ^
-o "%APPDATA%"/FIABiosum/tvbc_tree_data.db -ot tvbc_tree_data_calc
SET ERRNUM=%ERRORLEVEL%
...
:errdb
echo msgbox "SQLite database file not found",0,"FIA-TVBC Volume and Biomass Compiler" > %tmp%\biosum_err_msg.vbs
IF "%SHOWMSG%"=="Y" wscript %tmp%\biosum_err_msg.vbs
del %tmp%\biosum_err_msg.vbs
GOTO exit

:errtree
echo msgbox "Not a valid tree count",0,"FIA-TVBC Volume and Biomass Compiler" > %tmp%\biosum_err_msg.vbs
IF "%SHOWMSG%"=="Y" wscript %tmp%\biosum_err_msg.vbs
del %tmp%\biosum_err_msg.vbs
GOTO exit
...
echo msgbox "%ERRMSG%",0,"FIA-TVBC Volume and Biomass Compiler" > %tmp%\biosum_err_msg.vbs
IF "%SHOWMSG%"=="Y" wscript %tmp%\biosum_err_msg.vbs
del %tmp%\biosum_err_msg.vbs
GOTO exit
